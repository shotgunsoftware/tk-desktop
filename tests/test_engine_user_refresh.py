...
class TestEngineUserRefreshIntegration:
    ...
    def test_complete_user_change_flow(self, mock_sgtk, mock_auth):
        ...
        engine_impl.refresh_user_credentials = (
            lambda: DesktopEngineSiteImplementation.refresh_user_credentials(
                engine_impl
            )
        )
        engine_impl._check_and_update_current_user = lambda prev=None: DesktopEngineSiteImplementation._check_and_update_current_user(
            engine_impl, prev
        )
